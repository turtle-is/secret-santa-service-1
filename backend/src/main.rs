...
async fn main() -> Result<(), std::io::Error> {
    ...
    app.at("/create-group")
        .get(|mut request: Request<Arc<Mutex<DataBase>>>| async move {
            let AddGroup { name, creator } = request.body_json().await?;

            let state = request.state();
            let guard = state.lock().unwrap();

            eprintln!("Searching for group {name}");


            match guard.users.get(&creator) {
                None =>Err(tide::Error::from_str(
                    tide::StatusCode::Conflict,
                    format!("User {creator} not found or Group {name} already exist.")
                )),
                Some(user) => {
                    match guard.groups.get(&name) {
                        Some(..)=> Err(tide::Error::from_str(
                            tide::StatusCode::Conflict,
                            format!("User {creator} not found or Group {name} already exist."))),
                        None => Ok({
                            ...
                            guard.groups.insert(name.clone(), group);
                            guard.users.get_mut(&creator).unwrap().group = name.clone();
                        })
                    }
                }
            }.unwrap();
